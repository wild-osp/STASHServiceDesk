...
Проект теперь запускается так, что Telegram-бот поднимается автоматически вместе с API. Для хостинга достаточно запустить приложение через стартовый скрипт или Procfile — вручную запускать `python runner.py` больше не нужно.
